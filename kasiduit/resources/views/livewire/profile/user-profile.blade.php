    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-10">
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="w-full md:w-1/3 lg:w-1/4 flex flex-col gap-6">
                ...
                {{-- CARD FOTO PROFIL (UPDATED) --}}
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    
                    {{-- Area Foto dengan Alpine JS untuk Dropdown --}}
                    <div class="relative inline-block" x-data="{ open: false }">
                        
                        {{-- 1. TAMPILAN FOTO --}}
                        @if ($photo)
                            {{-- Preview foto upload baru --}}
                        ...
                        {{-- 2. TOMBOL KAMERA (Trigger Menu) --}}
                        <button @click="open = !open" 
                                @click.away="open = false"
                                class="absolute bottom-0 right-0 bg-gray-800 text-white p-2.5 rounded-full hover:bg-gray-700 transition border-4 border-white cursor-pointer shadow-sm z-20 flex items-center justify-center">
                        ...
                        <div x-show="open" 
                             ...
                             class="absolute bottom-12 -right-16 w-40 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-30 text-left overflow-hidden"
                             style="display: none;">
                            
                            {{-- Pilihan Ganti Foto --}}
                            <button @click="$refs.photoInput.click(); open = false" 
                                    class="w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center gap-2 transition font-medium">
                            ...
                            {{-- Pilihan Hapus Foto (Jika Ada) --}}
                            @if(Auth::user()->avatar)
                                <button wire:click="deletePhoto" 
                                        @click="open = false"
                                        wire:confirm="Yakin ingin menghapus foto profil?"
                                        class="w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 transition font-medium border-t border-gray-50">
